...
export const loginUser = async (req, res) => {
    try {
        ...
        console.log(error);
        return res.satus(400).json({status:false, message: error.message});
    }
}
...
export const getNotificationsList = async (req, res) => {
    try {
        ...
        const notice = await Notice.find({team: userId,isRead: { $nin: [userId] }}).populate('task', 'title');
...
export const updateUserProfile = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            res.status(201).json({status:true, message: 'Profile Updated Successfully', user: updatedUser});
        } else {
...
export const activateUserProfile = async (req, res) => {
    try {
        ...
        console.log(error);
        return res.status(400).json({status:false, message: error.essage});
    }
}
