...
export const createTask= async (req, res) => {
    try {
        ...
        const task = await Task.create({title, team, stage: stage.toLowerCase(), date, priority: priority.toLowerCase(), assets, activities: activity});
...
export const duplicateTask= async (req, res) => {
    try {
        ...
        res.status(200).json({status:true, task, message: 'Task duplicated successfully.'});
